...
namespace DataAccess.MongoDB
{
    ...
    public sealed class MGContext
    {
        ...
        public MGContext(string url, string db, string collection)
        {
        ...
        public MGContext(string collection)
        {
            if (ConfigHelper.UriMongDB_MasterData == null) throw new ArgumentNullException("url");
            MongoClient mgClient = new MongoClient(ConfigHelper.UriMongDB_MasterData);
            var db = mgClient.GetServer().GetDatabase(new MongoUrl(ConfigHelper.UriMongDB_MasterData).DatabaseName);
            this.collection = db.GetCollection(collection);
            this.gridfs = this.collection.Database.GetGridFS(MongoGridFSSettings.Defaults);

        }
        public MGContext(string collection,string sNameIndex)
        {
            if (ConfigHelper.UriMongDB_MasterData == null) throw new ArgumentNullException("url");
            MongoClient mgClient = new MongoClient(ConfigHelper.UriMongDB_MasterData);
            var db = mgClient.GetServer().GetDatabase(new MongoUrl(ConfigHelper.UriMongDB_MasterData).DatabaseName);
            this.collection = db.GetCollection(collection);
            //this.collection.CreateIndex(sNameIndex);
            this.collection.CreateIndex(new IndexKeysBuilder().Text(sNameIndex));
            this.gridfs = this.collection.Database.GetGridFS(MongoGridFSSettings.Defaults);
        ...
        public MGContext(MongoCollection collection)
        {
            if (collection == null) throw new ArgumentNullException("collection");
        ...
        public Message Get(QueryDocument query, TimeSpan resetRunning)
        {
            ...
            return Get(query, resetRunning, TimeSpan.FromMilliseconds(3));
        }
        ...
        public Message Get(QueryDocument query, TimeSpan resetRunning, TimeSpan wait)
        {
            ...
            return Get(query, resetRunning, wait, TimeSpan.FromMilliseconds(20));
        }
        ...
        public Message Get(QueryDocument query, TimeSpan resetRunning, TimeSpan wait, TimeSpan poll, bool approximateWait)
        {
            ...
            //collection.Update(
            //    new QueryDocument { { "running", true }, { "resetTimestamp", new BsonDocument("$lte", DateTime.Now) } },
            //    new UpdateDocument("$set", new BsonDocument("running", false)),
            //    UpdateFlags.Multi
            //);

            var builtQuery = new QueryDocument();
            foreach (var field in query)
                builtQuery.Add(field.Name, field.Value);

            //builtQuery.Add("earliestGet", new BsonDocument("$lte", DateTime.Now));
            ...
            //{
            //    resetTimestamp = resetRunning > TimeSpan.Zero ? DateTime.Now.AddYears(1) : DateTime.MinValue;
            //}
            ...
            while (true)
            {
                ...
                var findModifyArgs = new FindAndModifyArgs { Query = builtQuery,Update=null};
                var message = collection.FindAndModify(findModifyArgs).ModifiedDocument;
                //var message = collection.FindAndModify(new FindAndModifyArgs { Query = builtQuery, Update = update, Fields = fields, Upsert = false }).ModifiedDocument;
                if (message != null)
                {
        ...
        {
            AckSend(handle, payload, DateTime.Now);
        }
        ...
        public void Send(BsonDocument payload, DateTime earliestGet, double priority, IEnumerable<KeyValuePair<string, Stream>> streams)
        {
            ...
            var message = new BsonDocument
            {
                {"payload", payload},
                {"running", false},
                {"resetTimestamp", DateTime.Now.AddYears(1)},
                {"earliestGet", earliestGet},
                {"priority", priority},
