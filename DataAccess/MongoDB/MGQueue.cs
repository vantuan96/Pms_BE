...
namespace DataAccess.MongoDB
{
    ...
    public sealed class MGQueue
    {
        ...
        public MGQueue(string url, string db, string collection)
        {
        ...
        public MGQueue(string collection)
        {
            if (ConfigHelper.UriMongDB_Queue == null) throw new ArgumentNullException("url");
            MongoClient mgClient = new MongoClient(ConfigHelper.UriMongDB_Queue);
            var db = mgClient.GetServer().GetDatabase(new MongoUrl(ConfigHelper.UriMongDB_Queue).DatabaseName);
            this.collection = db.GetCollection(collection);
            this.gridfs = this.collection.Database.GetGridFS(MongoGridFSSettings.Defaults);

        }
        public MGQueue(string collection,string sNameIndex)
        {
            if (ConfigHelper.UriMongDB_Queue == null) throw new ArgumentNullException("url");
            MongoClient mgClient = new MongoClient(ConfigHelper.UriMongDB_Queue);
            var db = mgClient.GetServer().GetDatabase(new MongoUrl(ConfigHelper.UriMongDB_Queue).DatabaseName);
        ...
        public MGQueue(MongoCollection collection)
        {
            if (collection == null) throw new ArgumentNullException("collection");
        ...
        public Message Get(QueryDocument query, TimeSpan resetRunning, TimeSpan wait, TimeSpan poll, bool approximateWait)
        {
            ...
            while (true)
            {
                ...
                var findModifyArgs = new FindAndModifyArgs { Query = builtQuery, Update = update, Fields = fields, Upsert = false };
