...
{
    public class AutoMigrateCalculatePriceJob:IJob
    {
        ...
        public void Execute(IJobExecutionContext context)
        {
            ...
            CustomLog.intervaljoblog.Info($"<Auto Calculate price Inside setting policy> Start!");
            try
            {
                //var results = unitOfWork.PackageRepository.AsEnumerable().Where(x => !x.IsDeleted && x.IsActived == true && x.IsFromeHos == true);
                var results = new PackageRepo().GetPackagesForMigrate();
                CustomLog.intervaljoblog.Info(string.Format("<Auto Calculate price Inside setting policy> Total item: {0}", results?.Count()));
                ...
                foreach (var item in results)
                {
                    ...
                    if(temPackage==null)
                    {
                        CustomLog.intervaljoblog.Info(string.Format("<Auto MigrateCalculatePrice job> Package[Code={0}] is not found in Temp_Package", item.Code));
                        continue;
                    }
                    if (!string.IsNullOrEmpty(temPackage.Notes))
                    {
                        CustomLog.intervaljoblog.Info(string.Format("<Auto MigrateCalculatePrice job> Package[Code={0}] is have note", item.Code));
                        continue;
                    }
                    try {
                        if (item.PackagePrices?.Count > 0)
                        {
                            ...
                            if (groupPolicy.Any())
                            {
                                ...
                                foreach(var itemx in groupPolicy)
                                {
                                    ...
                                    //Get Price detail
                                    var entities = new PackageRepo().GeneratePackagePriceDetail(item, chargetypecode, pkgAmount, chargetypecode_fn, pkgAmount_fn,false,null);
                                    if (entities?.Count > 0)
                                    {
                                        ...
                                        if (existPkgPriceZero.Any())
                                        {
                                            ...
                                            CustomLog.intervaljoblog.Info(string.Format("<Auto MigrateCalculatePrice job> Package[Code={0}] is have service price is zero or null", item.Code));
                                        ...
                                        var countServiceInpackage = unitOfWork.ServiceInPackageRepository.Count(x => !x.IsDeleted && x.PackageId == packageId);
                                        if (countServiceInpackage != entities?.Count)
                                        {
                                            //Số lượng dịch vụ thiết lập giá và dịch vụ trong giá ko bằng nhau
                                            CustomLog.intervaljoblog.Info(string.Format("<Auto MigrateCalculatePrice job> Package[Code={0}] have service in package number is not equal service in policy setting", item.Code));
                                        ...
                                        CustomLog.intervaljoblog.Info(string.Format("<Auto CreateOrUpdatePricePolicyDetail> Package[Code={0}]", item.Code));
                                    }
                                    else
                                    {
                                        //Số lượng dịch vụ thiết lập giá không có
                                        CustomLog.intervaljoblog.Info(string.Format("<Auto MigrateCalculatePrice job> Package[Code={0}] have any more service in policy", item.Code));
                        ...
                        CustomLog.errorlog.Info(string.Format("<Auto MigrateCalculatePrice job> Package[Code={0}]. Ex: {1}", item.Code,ex.Message));
                ...
                CustomLog.Instant.IntervalJobLog($"<Auto MigrateCalculatePrice> Success!", Constant.Log_Type_Info, printConsole: true);
            }
            catch (Exception ex)
            {
                CustomLog.errorlog.Info(string.Format("<Auto Calculate price Inside setting policy> Error: {0}", ex));
            }
