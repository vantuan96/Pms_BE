...
{
    public class AutoRegPackageServiceJob : IJob
    {
        ...
        public void Execute(IJobExecutionContext context)
        {
            ...
            try
            {
                var results = unitOfWork.Temp_PatientInPackageRepository.AsEnumerable();
                CustomLog.intervaljoblog.Info(string.Format("<Auto Reg package service for patient> Total item: {0}", results?.Count()));
                foreach (var item in results)
                {
                    #region Comment lại có thể sử dụng sau
                    //for (int iOrder = 1; iOrder < item.RegCount + 1; iOrder++)
                    //{
                    ...
                    //        continue;
                    //    }
                    //    CustomLog.intervaljoblog.Info(string.Format("<Auto Reg package service for patient> Begin reg package [{0}] for patient [{1}], order [{2}]", item.PackageCode, item.PID, iOrder));
                    //    try {
                    ...
                    //            if (site == null)
                    //            {
                    //                CustomLog.intervaljoblog.Info(string.Format("<Auto Reg package service for patient> Reg package [{0}] for patient [{1}], order [{2}] Not Found Site", item.PackageCode, item.PID, iOrder));
                    ...
                    //            //Get Package info
                    //            var pkgEntity = unitOfWork.PackageRepository.Find(x => x.Code.ToLower() == item.PackageCode.ToLower()).FirstOrDefault();
                    //            if (pkgEntity == null)
                    //            {
                    //                CustomLog.intervaljoblog.Info(string.Format("<Auto Reg package service for patient> Reg package [{0}] for patient [{1}], order [{2}] Not Found Package", item.PackageCode, item.PID, iOrder));
                    ...
                    //            }
                    //            //Get PolicyId
                    //            var xPolicy = _repoPkg.PricePolicyAvailable(pkgEntity.Id, request.SiteCode, request.PersonalType, string.Empty);
                    //            if (!xPolicy.Any())
                    //            {
                    //                CustomLog.intervaljoblog.Info(string.Format("<Auto Reg package service for patient> Reg package [{0}] for patient [{1}], order [{2}] Not Found Policy", item.PackageCode, item.PID, iOrder));
                    //                item.Notes = !string.IsNullOrEmpty(item.Notes) ? string.Format("{0}. {1}", item.Notes, "Not Found Policy") : "Not Found Policy";
                    //                unitOfWork.Temp_PatientInPackageRepository.Update(item);
                    //                //unitOfWork.Commit();
                    //                continue;
                    //            }
                    ...
                    //            DateTime endate = dStartDate;
                    //            #region Comment rule cũ
                    //            //switch (item.UnitTimeUsing){
                    //            //    case "Tháng":
                    //            //        request.StartAt = iOrder==1?dStartDate.AddMonths((iOrder-1)*item.QtyTimeUsing).ToString(Constant.DATE_FORMAT): dStartDate.AddDays(iOrder-1).AddMonths((iOrder - 1) * item.QtyTimeUsing).ToString(Constant.DATE_FORMAT);
                    //            //        request.EndAt= request.GetStartAt().AddMonths(item.QtyTimeUsing).ToString(Constant.DATE_FORMAT);
                    //            //        break;
                    //            //    case "Năm":
                    //            //        request.StartAt = iOrder == 1 ? dStartDate.AddYears((iOrder - 1) * item.QtyTimeUsing).ToString(Constant.DATE_FORMAT): dStartDate.AddDays(iOrder - 1).AddYears((iOrder - 1) * item.QtyTimeUsing).ToString(Constant.DATE_FORMAT);
                    //            //        request.EndAt = request.GetStartAt().AddYears(item.QtyTimeUsing).ToString(Constant.DATE_FORMAT);
                    //            //        break;
                    //            //}
                    //            #endregion .Comment rule cũ
                    //            //Các gói của cùng 1 khách hàng có thể overlap & có thời hạn 5 năm
                    //            request.StartAt = dStartDate.ToString(Constant.DATE_FORMAT);
                    //            request.EndAt= dStartDate.AddYears(5).ToString(Constant.DATE_FORMAT);
                    ...
                    //            double netAmount = 0;
                    //            var entities = _repo.GetListPatientInPackageService(request.PolicyId.Value, policy.Amount.ToString(), out netAmount);
                    ...
                    //            {
                    //                CustomLog.intervaljoblog.Info(string.Format("<Auto Reg package service for patient> Reg package [{0}] for patient [{1}], order [{2}] Success", item.PackageCode, item.PID, iOrder));
                    //            }
                    //            else
                    //            {
                    ...
                    //        else
                    //        {
                    //            CustomLog.intervaljoblog.Info(string.Format("<Auto Reg package service for patient> Reg package [{0}] for patient [{1}], order [{2}] Not Found Patient", item.PackageCode, item.PID, iOrder));
                    ...
                    //    }
                    //}
                    #endregion .Comment lại có thể sử dụng sau
                }
                unitOfWork.Commit();
        ...
        private int CreateOrUpdatePatientInPackage(PatientInPackageModel request,bool isMigrateFromEhos, out PatientInPackage outOvelapPiPkg)
        {
            int returnValue = (int)StatusEnum.SUCCESS;
