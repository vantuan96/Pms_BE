...
{
    [ToolboxData("<{0}:SmartPager runat=\"server\"></{0}:SmartPager>")]
    public class SmartPager : WebControl, IPostBackEventHandler, INamingContainer
    {
        ...
        {
            string templateCell = "<a class=\"paginate_button\" href=\"{0}\" title=\"" + " " + BackToFirstClause + " " + "\"> " + FirstClause + " </a>";
            return String.Format(templateCell, Page.ClientScript.GetPostBackClientHyperlink(this, "1"));
        }

        private string RenderLast()
        {
            string templateCell = "<a class=\"paginate_button\" href=\"{0}\" title=\"" + " " + GoToLastClause + " " + "\"> " + LastClause + " </a>";
        ...
        {
            string templateCell = "<a class=\"paginate_button\" href=\"{0}\" title=\"" + " " + BackToPageClause + " " + (CurrentIndex - 1).ToString() + "\"> " + PreviousClause + " </a>";
        ...
        {
            string templateCell = "<a class=\"last paginate_button\" href=\"{0}\" title=\"" + " " + NextToPageClause + " " + (CurrentIndex + 1).ToString() + "\"> " + NextClause + " </a>";
            return String.Format(templateCell, Page.ClientScript.GetPostBackClientHyperlink(this, (CurrentIndex + 1).ToString()));
        }
        ...
        private string RenderCurrent()
        {
            if (CurrentIndex == PageCount)
                return "<a class=\"paginate_active\" style=\"border-right: 1px solid #d9d9d9;\"><span class=\"PagerHyperlinkStyle\" " + GetAlternativeText(CurrentIndex) + " ><strong> " + CurrentIndex.ToString() + " </strong></span></a>";
            else
                return "<a class=\"paginate_active\"><span class=\"PagerHyperlinkStyle\" " + GetAlternativeText(CurrentIndex) + " ><strong> " + CurrentIndex.ToString() + " </strong></span></a>";
        }

        private string RenderOther(int pageNumber)
        {
            string templateCell = "<a class=\"paginate_button\" href=\"{0}\" " + GetAlternativeText(pageNumber) + " > " + pageNumber.ToString() + " </a>";
        ...
        {
            string templateCell = "<a class=\"PagerSSCCells\"><a class=\"PagerHyperlinkStyle PagerHyperlinkHiden\" href=\"{0}\" " + GetAlternativeText(pageNumber) + " > " + pageNumber.ToString() + " </a></a>";
            return String.Format(templateCell, Page.ClientScript.GetPostBackClientHyperlink(this, pageNumber.ToString()));
        }

        private string RenderGoTo()
        {
            string templateCell = "<a style=\"padding:1px 1px 1px 1px;\" class=\"paginate_button\"><div onclick=\"handleGoToVisibility()\" class=\"GoToLabel\">&nbsp;Go To&nbsp;</div><img id=\"goto_img\" onclick=\"handleGoToVisibility()\" src=\"" + Page.ClientScript.GetWebResourceUrl(this.GetType(), "ASPnetPagerV2_8.Images.arr_right.gif") + "\" alt=\"\" class=\"GoToArrow\"/>&nbsp;<div id=\"div_goto\" style=\"display:none;\"><select class=\"GoToSelect\" name=\"ddlTes\" id=\"ddlTes\" onchange=\"javascript:handleGoto(this);\">{0}</select></div></a>";
        ...
        protected override void Render(HtmlTextWriter writer)
        {
            ...
            writer.AddAttribute(HtmlTextWriterAttribute.Class, "dataTables_paginate paging_full_numbers");
            ...
            {
                writer.AddAttribute(HtmlTextWriterAttribute.Class, "PagingInfor");
