...
* 
* 06/01/2008 
*  - Better support for white space in JSON
*  - Updated ParseArray Routine based on fixes from
*    Joe McLain to handle arrays of any type.
***************************************************************/
...
namespace GAPIT.MKT.Framework.Core.JSON
{
    ...
    public class WestwindJsonSerializer : IJSONSerializer
    {
        ...
        public string ParseString(string value)
        {
            ...
            if (value == null)
                throw new ArgumentException(WebConsts.ERROR_INVALID_JSON_STRING);
        ...
        private DateTime ParseDate(string jsonDate)
        {
            ...
            try
            {
                ...
                if (jsonDate.StartsWith(@"\/Date(", StringComparison.Ordinal))
                {
                    ...
                    jsonDate = StringHelper.ExtractString(jsonDate, @"\/Date(", @")\/");
                ...
                {
                    DateTime date = DateTime.Parse(jsonDate, CultureInfo.InvariantCulture.DateTimeFormat, DateTimeStyles.RoundtripKind).ToLocalTime();
                    return date;
                ...
                jsonDate = StringHelper.ExtractString(jsonDate, "new Date(", ")");
