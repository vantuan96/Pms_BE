...
{
    /// <summary>
    /// Represents an xml root CPCatalog document element.
    /// </summary>
    [XmlRoot("CPCatalog")]
    public class CPCatalog
    {
        ...
        public CPService FindServiceByID(string serviceID)
        {            
            foreach (CP c in CPs)
            {
                if (c.CPServices!=null)
                {
                    foreach (CPService cs in c.CPServices.Services)
                    {
                        if (cs.ID == serviceID)
                        {
                            return cs;
